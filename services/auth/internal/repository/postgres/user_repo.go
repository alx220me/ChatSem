...
// UserRepo implements domain.UserRepository for the auth service using pgx.
...
// On conflict it updates the name (role changes come from admin service).
func (r *UserRepo) Upsert(ctx context.Context, u *domain.User) (*domain.User, error) {
	slog.Debug("[AuthUserRepo.Upsert] upsert", "external_id", u.ExternalID, "event_id", u.EventID)
	...
	if err := row.Scan(&result.ID, &result.ExternalID, &result.EventID, &result.Name, &result.Role, &result.CreatedAt); err != nil {
		return nil, fmt.Errorf("AuthUserRepo.Upsert: %w", err)
	}
	slog.Debug("[AuthUserRepo.Upsert] done", "user_id", result.ID, "role", result.Role)
	return result, nil
}

// GetByID returns the user with the given ID.
func (r *UserRepo) GetByID(ctx context.Context, id uuid.UUID) (*domain.User, error) {
	slog.Debug("[AuthUserRepo.GetByID] query", "user_id", id)
	row := r.db.QueryRow(ctx, `
	...
	if err := row.Scan(&u.ID, &u.ExternalID, &u.EventID, &u.Name, &u.Role, &u.CreatedAt); err != nil {
		return nil, fmt.Errorf("AuthUserRepo.GetByID: %w", err)
	}
	return u, nil
}
...
func (r *UserRepo) GetByExternalID(ctx context.Context, externalID string, eventID uuid.UUID) (*domain.User, error) {
	slog.Debug("[AuthUserRepo.GetByExternalID] query", "external_id", externalID, "event_id", eventID)
	row := r.db.QueryRow(ctx, `
		SELECT id, external_id, event_id, name, role, created_at
	...
	u := &domain.User{}
	if err := row.Scan(&u.ID, &u.ExternalID, &u.EventID, &u.Name, &u.Role, &u.CreatedAt); err != nil {
		return nil, fmt.Errorf("AuthUserRepo.GetByExternalID: %w", err)
	}
	return u, nil
}
...
func (r *UserRepo) ListByEventID(ctx context.Context, eventID uuid.UUID, limit, offset int) ([]*domain.User, error) {
	slog.Debug("[AuthUserRepo.ListByEventID] query", "event_id", eventID)
	rows, err := r.db.Query(ctx, `
		SELECT id, external_id, event_id, name, role, created_at
		FROM users WHERE event_id = $1 ORDER BY created_at ASC LIMIT $2 OFFSET $3`,
		eventID, limit, offset)
	if err != nil {
		return nil, fmt.Errorf("AuthUserRepo.ListByEventID: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&u.ID, &u.ExternalID, &u.EventID, &u.Name, &u.Role, &u.CreatedAt); err != nil {
			return nil, fmt.Errorf("AuthUserRepo.ListByEventID scan: %w", err)
		}
...
func (r *UserRepo) UpdateRole(ctx context.Context, id uuid.UUID, role domain.UserRole) error {
	slog.Debug("[AuthUserRepo.UpdateRole] update", "user_id", id, "role", role)
