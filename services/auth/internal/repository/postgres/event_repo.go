...
// EventRepo implements domain.EventRepository for the auth service using pgx.
...
func (r *EventRepo) GetByID(ctx context.Context, id uuid.UUID) (*domain.Event, error) {
	slog.Debug("[AuthEventRepo.GetByID] query", "event_id", id)
...
func (r *EventRepo) Create(ctx context.Context, e *domain.Event) error {
	slog.Debug("[AuthEventRepo.Create] inserting event", "name", e.Name)
	row := r.db.QueryRow(ctx, `
	...
	if err := row.Scan(&e.ID, &e.CreatedAt); err != nil {
		return fmt.Errorf("AuthEventRepo.Create: %w", err)
	}
	slog.Debug("[AuthEventRepo.Create] done", "event_id", e.ID)
	return nil
}

// List returns all events ordered by creation time.
func (r *EventRepo) List(ctx context.Context) ([]*domain.Event, error) {
	slog.Debug("[AuthEventRepo.List] query")
	rows, err := r.db.Query(ctx, `
		SELECT id, name, settings, allowed_origin, api_secret, created_at
		FROM events ORDER BY created_at ASC`)
	if err != nil {
		return nil, fmt.Errorf("AuthEventRepo.List: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.ID, &e.Name, &e.Settings, &e.AllowedOrigin, &e.APISecret, &e.CreatedAt); err != nil {
			return nil, fmt.Errorf("AuthEventRepo.List scan: %w", err)
		}
		events = append(events, e)
	}
	slog.Debug("[AuthEventRepo.List] fetched", "count", len(events))
