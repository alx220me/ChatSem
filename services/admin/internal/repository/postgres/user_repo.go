...
// UserRepo implements domain.UserRepository for the admin service using pgx.
...
func (r *UserRepo) Upsert(ctx context.Context, u *domain.User) (*domain.User, error) {
	slog.Debug("[AdminUserRepo.Upsert] upsert", "external_id", u.ExternalID)
	row := r.db.QueryRow(ctx, `
		INSERT INTO users (external_id, event_id, name, role)
		VALUES ($1, $2, $3, $4)
		ON CONFLICT (external_id, event_id) DO UPDATE SET name=EXCLUDED.name
	...
	if err := row.Scan(&result.ID, &result.ExternalID, &result.EventID, &result.Name, &result.Role, &result.CreatedAt); err != nil {
		return nil, fmt.Errorf("AdminUserRepo.Upsert: %w", err)
	}
	return result, nil
}
...
func (r *UserRepo) GetByID(ctx context.Context, id uuid.UUID) (*domain.User, error) {
	slog.Debug("[AdminUserRepo.GetByID] query", "user_id", id)
	row := r.db.QueryRow(ctx, `
	...
	if err := row.Scan(&u.ID, &u.ExternalID, &u.EventID, &u.Name, &u.Role, &u.CreatedAt); err != nil {
		return nil, fmt.Errorf("AdminUserRepo.GetByID: %w", err)
	}
	return u, nil
}
...
func (r *UserRepo) GetByExternalID(ctx context.Context, externalID string, eventID uuid.UUID) (*domain.User, error) {
	slog.Debug("[AdminUserRepo.GetByExternalID] query", "external_id", externalID)
	row := r.db.QueryRow(ctx, `
		SELECT id, external_id, event_id, name, role, created_at
	...
	u := &domain.User{}
	if err := row.Scan(&u.ID, &u.ExternalID, &u.EventID, &u.Name, &u.Role, &u.CreatedAt); err != nil {
		return nil, fmt.Errorf("AdminUserRepo.GetByExternalID: %w", err)
	}
	return u, nil
}
...
func (r *UserRepo) ListByEventID(ctx context.Context, eventID uuid.UUID, limit, offset int) ([]*domain.User, error) {
	slog.Debug("[AdminUserRepo.ListByEventID] query", "event_id", eventID, "limit", limit)
	rows, err := r.db.Query(ctx, `
		SELECT id, external_id, event_id, name, role, created_at
		FROM users WHERE event_id=$1 ORDER BY created_at ASC LIMIT $2 OFFSET $3`,
		eventID, limit, offset)
	if err != nil {
		return nil, fmt.Errorf("AdminUserRepo.ListByEventID: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&u.ID, &u.ExternalID, &u.EventID, &u.Name, &u.Role, &u.CreatedAt); err != nil {
			return nil, fmt.Errorf("AdminUserRepo.ListByEventID scan: %w", err)
		}
...
func (r *UserRepo) UpdateRole(ctx context.Context, id uuid.UUID, role domain.UserRole) error {
	slog.Debug("[AdminUserRepo.UpdateRole] update", "user_id", id, "role", role)
	_, err := r.db.Exec(ctx, `UPDATE users SET role=$2 WHERE id=$1`, id, role)
	if err != nil {
		return fmt.Errorf("AdminUserRepo.UpdateRole: %w", err)
	}
	return nil
}
